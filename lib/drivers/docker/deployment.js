...
module.exports = exports = Deployment;

function Deployment(id, baseDir) {
...
inherits(Deployment, EventEmitter);

Deployment.prototype.receive = function(req, res) {
...
Deployment.prototype._onCommit = function(commit) {
  var imgOpts = {
    appRoot: commit.dir,
    imgName: fmt('%s/%s:%s', 'strong-pm', this.instanceId, commit.hash),
  };
  var self = this;
  this.debug('Deployment committed', commit);
  this.commit = commit;
...
Deployment.prototype._onPrepared = function(commit) {
  this.debug('Deployment prepared', commit);
};
