...
Image.prototype._onCommit = function(commit) {
  this.name = fmt('%s/%s:%s', 'strong-pm', this.instanceId, commit.hash);
  var self = this;
...
Image.prototype.start = function(logger, startOpts) {
  var container = new Container(this, logger, startOpts);
  container.on('runnable', function() {
    container.start();
  });
  return container;
};
