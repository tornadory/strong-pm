...
DockerDriver.prototype.stopInstance = function(id, style, cb) {
  ...
  // 1. lookup container by $id
  // 2. send stop-$style message via control channel
  // 3. cb with response
  setImmediate(cb);
  // cb(null, id, style);
};
...
DockerDriver.prototype.stop = function(cb) {
  console.error('DockerDriver.stop(%s)', cb.name);
  // 1. connecto to docker
  // 2. generate inventory list
  // 3. callback with inventore
  setImmediate(cb, this.state);
};
