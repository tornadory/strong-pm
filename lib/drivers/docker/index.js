...
function DockerDriver(opts) {
  ...
  this.on('container-started', this._onContainerStarted.bind(this));
  this.on('container-stopped', this._onContainerStopped.bind(this));
}
...
DockerDriver.prototype.deployInstance = function(id, req, res) {
  ...
  var image = new Image(id, this.baseDir);
  ...
  this.server.getInstanceEnv(id, function(err, env) {
    ...
    sdr.run(img.name, args, {env: dockerEnv(env)}, function(err, app) {
      ...
      setImmediate(function() {
        self.emit('container-started', id, app);
      });
...
DockerDriver.prototype._onContainerStarted = function(id, app) {
  var self = this;
  var attachOpts = {stream: true, stdout: true, stderr: true};
  var logger = app.logger = app.logger || bl();
  logger.logGcTimer = setInterval(gcLogger, exports.LOG_GC_INTERVAL_MS);
  logger.logGcTimer.unref();
  console.log('attaching to instance %j:', id, app);
  app.container.attach(attachOpts, function(err, stream) {
    if (err) {
      clearInterval(logger.logGcTimer);
      console.error('error attaching to container %j:', id, app, err);
      self.stopInstance(id, 'hard', function(err, res) {
        console.error('error stopping container %j:', id, app, res, err);
        self.startInstance(id, function() {
          console.log('restarted instance %j', id);
        })
      });
      return;
    }
    app.container.modem.demuxStream(stream, logger, logger);
  ...
  function reconnect(err) {
    console.error('lots connection to instance, reconnecting to %j:', id, err);
    clearInterval(logger.logGcTimer);
    self._onContainerStarted(id, app);
  }
};
...
DockerDriver.prototype.dumpInstanceLog = function(id) {
  ...
  var logDump = current.app.logger.duplicate().toString();
  current.app.logger.consume(logDump.length);
  return logDump;
};
