  ...
  this.server.getInstanceEnv(id, function(err, env) {
    ...
    sdr.run(img.name, args, {env: dockerEnv(env)}, function(err, app) {
      ...
      if (previous && previous.app && previous.app.container) {
        // TODO(rmg): remove previous processes from Instance instance,
        //            probably by faking a kill or exit notification somewhere
        previous.app.container.stop({t: 1}, function(err, res) {
