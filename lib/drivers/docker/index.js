...
DockerDriver.prototype._onContainerStarted = function(id, app) {
  console.assert(!app.logger, 'app should not already have a logger');
  var self = this;
  var attachOpts = {stream: true, stdout: true, stderr: true};
  app.logger = bl();
  app.logGcTimer = setInterval(gcLogger, exports.LOG_GC_INTERVAL_MS);
  app.logGcTimer.unref();
  app.container.attach(attachOpts, function(err, stream) {
    app.container.modem.demuxStream(stream, app.logger, app.logger);
    stream.on('close', function() {
      clearInterval(app.logGcTimer);
      self.emit('container-stopped', id, app);
    });
  });

  function gcLogger() {
    var overflow = app.logger.length - exports.MAX_LOG_RETENTION_BYTES;
    if (overflow > 0) {
      app.logger.consume(overflow);
    }
  }
};
