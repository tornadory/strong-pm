...
Runner.prototype.start = function start() {
  ...
  // set PWD in env (as shell does) to the working directory.
  var env = {
    PWD: path.resolve(commit.dir, '..', 'current'),
  }

  // Use npmPath.PATH instead of 'PATH' as key, takes care of OS differences
  // Generate $PATH that includes bins available to strong-pm, such as sl-run
  env[npmPath.PATH] = npmPath.get({ cwd: path.resolve(__dirname, '..') });
  // Augment $PATH so that it also includes bins available to app being run
  env[npmPath.PATH] = npmPath.get({ cwd: env.PWD, env: env });
  ...
  this.ctl = childctl.attach(function(){}, this.child);
